...
class IncomeViewModel : ViewModel() {
    ...
    fun setListIncomeByFilter(partnerId: String, filterDate: String) {
        listItems.clear()
    ...
    fun setListIncomeByFilterAdmin(dateStart: Long, dateFinish: Long) {
        listItems.clear()


        try {
            FirebaseFirestore.getInstance().collection("income")
                .whereGreaterThanOrEqualTo("dateTimeInMillis", dateStart)
                .whereLessThanOrEqualTo("dateTimeInMillis", dateFinish)
                .get()
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        val model = IncomeModel()
